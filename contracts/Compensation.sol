...
contract Compensation is Ownable {
  ...
  uint256 public totalTokensCompensation;
	...
	event Refill(address _owner, uint256 compensationPerRound, uint256 _totalAvailable);
	event Claim(address _receiver, uint256 _amount);
  event NextRound(uint256 currentRound);

	constructor(address _token, uint totalTokensCompensation, uint totalRounds)
		public
	{
		CompToken = IERC20(_token);
    compensationPerRound = totalTokensCompensation.div(totalRounds);
	}
	...
	*/
	function claimCompensation()
		public
	{
		uint256 claimAmount = tokenClaimLimit[msg.sender].div(totalRounds).mul(currentRound).sub(tokensClaimed[msg.sender]);
	...
	*/
	...
	{
		require(
			CompToken.transferFrom(msg.sender, address(this), compensationPerRound),
